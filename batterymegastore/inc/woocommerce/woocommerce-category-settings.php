	...
	if ( is_shop() && ! is_search() && ! is_paged() ) {
		...
		if ( ! empty( $content[0] ) ) {
			...
			<div class="thb-category-top-content" id="thb-category-top-content">
				<?php echo do_shortcode( wp_specialchars_decode( nl2br( $content[0] ) ) ); ?>
			</div>
			<?php
		}
	}
	...
	if ( is_shop() && ! is_search() && ! is_paged() ) {
		...
		if ( ! empty( $content[0] ) ) {
			...
			<div class="thb-category-bottom-content" id="thb-category-bottom-content">
				<?php echo wp_kses_post( do_shortcode( wp_specialchars_decode( nl2br( $content[0] ) ) ) ); ?>
			</div>
			<?php
		}
	}
...
function thb_edit_category_header_img( $term, $taxonomy ) {
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php esc_html_e( 'Taxonomy Top Content', 'peakshops' ); ?></label></th>
		<td>
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php esc_html_e( 'Taxonomy Bottom Content', 'peakshops' ); ?></label></th>
		<td>
