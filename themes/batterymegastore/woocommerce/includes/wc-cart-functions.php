	...
	if ( wc_tax_enabled() && WC()->cart->display_prices_including_tax() ) {
		...
		if ( ! empty( $tax_string_array ) ) {
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) {
				...
				//$tax_text = wp_kses_post( sprintf( __( '(incl %1$s estimated for %2$s)', 'woocommerce' ), implode( ', ', $tax_string_array ), $country ) );
				$tax_text = wp_kses_post( sprintf( __( '(incl %1$s estimated for %2$s)', 'woocommerce' ), implode( ', ',  ), $country ) );
			} else {
				/* translators: %s: tax amount */
				$tax_text = wp_kses_post( sprintf( __( '(incl + vat%s)', 'woocommerce' ), implode( ', ',  ) ) );
			}
			
			$value .= '<br /><small class="includes_tax">' .  $tax_text . '</small>';
		}
	}
